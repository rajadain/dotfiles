#!/usr/bin/fish
function settheme -d "Halts current vagrant, ups another"
    set -l theme $argv[1]

    if test $theme = "light"
        kitty @ set-colors ~/.config/kitty/piatto-light.conf
        ...
        kitty @ set-colors ~/.config/kitty/tomorrow-night-bright.conf
        set -U fish_color_normal normal
        ...
        echo "Unsupported theme $theme. Must be one of: light, dark"
    end
end
